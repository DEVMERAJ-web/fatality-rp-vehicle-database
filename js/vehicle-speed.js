document.addEventListener('DOMContentLoaded', () => {
  ...
  const ranges = [
    ...
    ['0-50', '0–50 km/h'],
    ['51-100', '51–100 km/h'],
    ['101-150', '101–150 km/h'],
    ['151-200', '151–200 km/h'],
    ['201-250', '201–250 km/h'],
    ['251-300', '251–300 km/h'],
    ['301-350', '301–350 km/h'],
    ['351-400', '351–400 km/h'],
    ['401+', '401+ km/h'],
    ['UNKNOWN', '❓ Speed Not Verified']
  ];
  ...
  function ensureSpeedFilter() {
    const panel = document.querySelector('.filter-panel');
    if (!panel) return null;

    let select = document.getElementById('speed-filter');
    if (!select) {
      const group = document.createElement('div');
      group.className = 'filter-group speed-filter-group';
      group.innerHTML = '<label for="speed-filter">Max Speed</label><select id="speed-filter" aria-label="Filter vehicles by maximum reference speed"></select><small class="speed-help">Reference top speed. Custom FiveM handling may differ.</small>';
      const favGroup = document.getElementById('favorites-toggle')?.closest('.filter-group');
      if (favGroup) panel.insertBefore(group, favGroup);
      else panel.appendChild(group);
      select = group.querySelector('#speed-filter');
    }

    const old = select.value || speedFilter;
    select.innerHTML = ranges.map(([value, label]) => `<option value="${value}">${label}</option>`).join('');
    select.value = ranges.some(([value]) => value === old) ? old : 'ALL';
    speedFilter = select.value;

    if (!select.dataset.bound) {
      select.addEventListener('change', event => {
        speedFilter = event.target.value;
        applySpeedFilter();
      });
  ...
  fetch(speedUrl)
    .then(r => {
      if (!r.ok) throw new Error(`Speed database HTTP ${r.status}`);
      return r.json();
    })
    .then(data => {
      ...
      if (grid) {
        // Watch only direct card insertion/removal. Watching the full subtree caused
        // a feedback loop because decorating cards changes their DOM.
    ...
    .catch(error => {
      console.error('Fatality speed database failed to load:', error);
      ensureSpeedFilter();
    });
});
